...
## Next we trimmed reads with ```trimmomatic v.40```. See trim_reads.sh 
...
## Next we mapped reads to the _P. relampaga_ reference genome (GCAXXXXXXX) using ```bwa 0.7.19```. See map_reads_sam_to_bam.sh 
```console
...
```  
We then assesed the mapping statisitcs, filtered low quality alignments (those with mapping quality less than 20), and removed duplicates using ```sambamba``` and ```gatk```. See mapping_stats_and_filtering.sh 
```console
...
```  
Lastly we estimated mean coverage of each individual using ```bedtools```. See calc_coverage.sh
```console
bedtools genomecov \
